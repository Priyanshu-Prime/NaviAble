...
NAVIABLE_DEMO_MODE="${NAVIABLE_DEMO_MODE:-true}"
...
NAVIABLE_DEMO_MODE="$NAVIABLE_DEMO_MODE" ENABLE_HYBRID_CLIP="$ENABLE_HYBRID_CLIP" python3 -m uvicorn app.main:app --host "$BACKEND_HOST" --port "$BACKEND_PORT" &
BACKEND_PID=$!
popd >/dev/null
